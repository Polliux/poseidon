...
func on_buy_attempt() -> bool:
	if (EventController.get_ui_control().get_resource_value("SCIENCE") >= card_res.science_buy_cost):
		EventController.get_ui_control().modify_resource_value("SCIENCE",card_res.science_buy_cost,-1)
		self.discard()
		return true
	else:
		return false
